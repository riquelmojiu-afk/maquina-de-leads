...
async function runMining(
  campaign: { id: number; nicho: string; cidades: string; name: string; searchVariations?: string | null },
  apiKey: string,
  logId: number,
  ...
  // Parse search variations if available
  let searchQueries: string[] = [];
  if (campaign.searchVariations) {
    try {
      searchQueries = JSON.parse(campaign.searchVariations);
    } catch {
      searchQueries = [];
    }
  }

  // If no variations, use default query with nicho
  if (searchQueries.length === 0) {
    searchQueries = [campaign.nicho];
  }

  log(progress, "info", `🔎 Estratégia de busca: ${searchQueries.length} variação(ões) de termo`);
  ...
  for (const city of cities) {
    for (const searchTerm of searchQueries) {
          ...
          for (const place of results) {
            ...
            try {
              const detailParams = new URLSearchParams({
                ...
                fields: "name,formatted_phone_number,international_phone_number,website,types,formatted_address,geometry",
                key: apiKey,
                language: "pt-BR",
              });
              ...
              // Check if phone already exists (global deduplication)
              ...
              log(
                progress,
                "success",
                `  ✅ Lead salvo: ${place.name} | ${phone || "sem telefone"} | ${extractedCity}`
              );
            } catch (err: unknown) {
      ...
      log(
        progress,
        "info",
        `✨ Termo "${searchTerm}" em "${city}": ${cityLeads} leads capturados em ${pageCount} página(s)`
      );
    }
  }
